...
interface TopArtistsResponse {
   ...
   * rather than ours, so the body is what gets believed below.
   */
  error?: number;
  message?: string;
}
 ...
 * no longer exists. An empty list means the user is real but has never
 * scrobbled anything, which reads differently on screen and so is kept
 * distinct from the miss.
 *
 * Throws only when Last.fm itself fails, which the route turns into a 500
 * rather than blaming the visitor for.
 */
export async function topArtists(user: string, limit: number): Promise<string[] | null> {
  ...
  const data = (await response.json().catch(() => null)) as TopArtistsResponse | null;
  ...
  for (const artist of data.topartists?.artist ?? []) {
    ...
    /*
     * Last.fm's catalogue is user-written, so it carries the junk that
     * implies. These two are its own placeholders for a scrobble whose
     * artist tag was empty, and they resolve to nothing downstream.
     */
    if (!name || name === "[unknown]" || name.toLowerCase() === "various artists") continue;
